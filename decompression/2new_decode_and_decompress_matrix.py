    ...
    with open(file_path, 'rb') as f:
        ...
        for _ in range(num_placeholder_positions):
            try:
                ...
                print(f"警告: 文件 {os.path.basename(file_path)} 在读取数据时提前结束。文件可能已损坏或不完整。")
                return num_placeholder_positions, num_instances, None
...
# 处理传统格式文件
def process_traditional_file(file_path, length_specs, subtoken_dict):
    """
    处理传统格式的bin文件
    :param file_path: 文件路径
    :param length_specs: 长度规格列表
    ...
    # 验证长度规格与位置数是否匹配
    if len(length_specs) != num_positions:
        print(f"警告: 长度规格数 ({len(length_specs)}) 与占位符位置数 ({num_positions}) 不匹配")
    ...
    for values in instance_values:
        ...
        for idx, v in enumerate(values):
            ...
            if idx < len(length_specs):
                ...
                if length_spec > 0:
                    # 固定长度：前置补零或截断
                    if value_str.isdigit() and len(value_str) < length_spec:
                        value_str = value_str.zfill(length_spec)
                    elif len(value_str) > length_spec:
                        value_str = value_str[:length_spec]
...
def load_subtoken_dictionary(dict_dir):
    """从目录中加载子标记字典"""
    dict_files = glob.glob(os.path.join(dict_dir, 'token.txt'))
    if not dict_files:
...
def main():
    ...
    # 处理所有bin文件
    bin_files = glob.glob(os.path.join(args.data_dir, '_*.bin'))
    subtoken_data = {}
    ...
    for bin_file in bin_files:
        ...
        if id_num is not None and id_num in composite_subtokens:
            ...
            if is_delta_file:
                ...
                placeholder_data = process_traditional_file(bin_file, length_specs, subtoken_dict)
